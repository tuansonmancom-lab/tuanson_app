...
def get_db_connection():
  """Connects to Turso Cloud (if credentials are provided) or falls back to local SQLite."""
  if HAS_LIBSQL and TURSO_DATABASE_URL:
    ...
    try:
      conn.sync()  # Sync remote data locally if using embedded replica configuration
    except Exception:
      pass
    return conn
  else:
    # Fallback to standard local SQLite database
...
def init_db():
  ...
  if c.fetchone()[0] == 0:
    ...
    c.executemany(
        (
            "INSERT INTO activities (project_id, activity_name, qty, unit,"
            " contract_amount) VALUES (?, ?, ?, ?, ?)"
        ),
        [
  ...
  if c.fetchone()[0] == 0:
    c.executemany(
        "INSERT INTO signatories (name, role, signature_path) VALUES (?, ?, ?)",
        [
...
def create_po_pdf(pono, date_str, supplier, project, po_items):
      ...
      for name, role, sig_path in sigs:
        if role and role.strip().lower() == "preparer":
          if name:
            prep_name = name
          if sig_path:
            prep_sig_path = sig_path.strip()
        elif role and role.strip().lower() == "approver":
          if name:
            appr_name = name
          if sig_path:
            appr_sig_path = sig_path.strip()
  except Exception:
    pass
  ...
  cell_style = ParagraphStyle(
      "CellBody", parent=styles["Normal"], fontName="Helvetica", fontSize=8, leading=10
  )
  ...
  company_text = Paragraph(
      "<b>TUANSON CONSTRUCTION</b> <font size=7 color='black'>162 P. Labuca"
      " St., Cansojong, Talisay City, Cebu Tel: 032 273-1187</font><br/>"
      "<font size=7 color='black'>web: www.tuansoncons.com, email:"
      " ric_tuanson@yahoo.com.ph</font>",
      company_style,
  )
  ...
  title_table = Table(
      [[
          Paragraph("<b>PURCHASE ORDER</b>", po_title_style),
          Paragraph(
              f"P.O. NO. &nbsp;&nbsp;&nbsp;&nbsp; <b>{pono}</b>", po_no_style
          ),
      ]],
      colWidths=[340, 200],
  )
  ...
  meta_data = [
      [
          Paragraph("PAYEE:", meta_label_style),
          Paragraph(str(supplier), meta_val_style),
          Paragraph("DATE:", meta_label_style),
          Paragraph(str(date_str), meta_val_style),
      ],
      [
          Paragraph("PROJECT NAME:", meta_label_style),
          Paragraph(str(project), meta_val_style),
          "",
          "",
      ],
  ]
  ...
  prep_element = Paragraph(
      f"<b>{prep_name}</b><br/>Prepared by:", center_cell_style
  )
  if prep_sig_path and os.path.exists(prep_sig_path):
    prep_img = Image(prep_sig_path, width=90, height=30)
    prep_element = [
        prep_img,
        Paragraph(f"<b>{prep_name}</b><br/>Prepared by:", center_cell_style),
    ]

  appr_element = Paragraph(
      f"<b>{appr_name}</b><br/>Approved by:", center_cell_style
  )
  if appr_sig_path and os.path.exists(appr_sig_path):
    appr_img = Image(appr_sig_path, width=90, height=30)
    appr_element = [
        appr_img,
        Paragraph(f"<b>{appr_name}</b><br/>Approved by:", center_cell_style),
    ]
  ...
  footer_data = [
      ...
      ["", "", prep_element, appr_element],
      [
          "",
          "",
          "",
          Paragraph(
              f"<font size=6 color='gray'>Date Print:"
              f" {current_time_str}</font>",
              right_cell_style,
          ),
      ],
  ]
...
if not st.session_state.logged_in:
  ...
  with st.form("login_form"):
    ...
    if submit_btn:
      user_data = c.execute(
          (
              "SELECT id, username, password, role1, role2, role3, role4, role5,"
              " role6, status, can_add_act, can_add_item FROM users WHERE"
...
role = st.sidebar.selectbox(
    "🔑 Select Your Active Role", st.session_state.available_roles
)
...
if role == "Requisitor":
  ...
  tab_request, tab_track = st.tabs(
      ["📝 New Material Request", "🔍 Track My Requests"]
  )
  ...
  with tab_request:
    ...
    projects = [
        r[0] for r in c.execute("SELECT project_name FROM projects").fetchall()
    ]
    ...
    activities = [
        r[0]
        for r in c.execute(act_query, (selected_project,)).fetchall()
    ]
    ...
    materials = c.execute(
        (
            "SELECT item_no, description, unit, COALESCE(category, 'Direct"
            " Materials') FROM materials"
        )
    ).fetchall()
    mat_options = (
        {
            f"[{m[0]}] {m[1]}": (m[0], m[1], m[2], m[3])
            for m in materials
        }
        if materials
        else {}
    ...
    if selected_mat_label == "➕ Add New Item...":
      ...
      item_no = col_ni1.text_input(
          "New Item Number (e.g., 00007 or custom code)"
      )
      ...
      item_no, description, default_unit, default_category = (
          "",
          "",
          "PCS",
          "Direct Materials",
      ...
      st.warning(
          "⚠️ No materials exist in the database yet. Contact an admin to add"
          " items."
      )
    else:
      item_no, description, default_unit, default_category = mat_options[
          selected_mat_label
      ]

    suppliers = [
        r[0]
        for r in c.execute("SELECT supplier_name FROM suppliers").fetchall()
    ]
    ...
    with st.form("request_form"):
      ...
      category = col_cat.selectbox(
          "Category (Accounting Tag)", category_options, index=cat_index
      )
      ...
      price = col2.number_input(
          "Estimated Price (Optional)", min_value=0.0, step=10.0
      )
      suggested_supplier = col3.selectbox(
          "Suggested Supplier (Optional)", supplier_options, index=0
      )
      ...
      if add_to_list:
        if selected_mat_label == "No items available":
          st.error("⚠️ Cannot add to list. No materials selected.")
            ...
            selected_activity_option == "➕ Add New Activity..."
            and not selected_activity
        ):
          st.error("⚠️ Please type a valid name for the new activity.")
        elif selected_mat_label == "➕ Add New Item..." and (
            not item_no.strip() or not description.strip()
        ):
          st.error(
              "⚠️ Please provide both a valid Item Number and Description for"
              " the new item."
          )
        else:
          if selected_activity_option == "➕ Add New Activity...":
            proj_id_row = c.execute(
          ...
          st.success(
              f"Added {qty} {unit} of {description} to your list!"
          )
          st.rerun()
    ...
    if len(st.session_state.request_cart) > 0:
      ...
      st.info(
          "💡 You can edit quantities, change categories, or delete rows before"
          " finalizing."
      )

      cart_df = pd.DataFrame(st.session_state.request_cart)
      edited_cart_df = st.data_editor(
          cart_df, num_rows="dynamic", use_container_width=True, key="cart_editor"
      )
  ...
  with tab_track:
    ...
    st.info(
        "Track the live status of all your submitted material requests here."
    )
...
# --- ROLE 2: PURCHASER ---
  ...
  tab_create_po, tab_receive = st.tabs(
      ["📝 Create Purchase Orders", "📦 Receive Deliveries"]
  )
  ...
  with tab_create_po:
    ...
    pending_df = pd.read_sql_query(
        (
            "SELECT id, project_name, activity, item_no, description, qty,"
            " unit, price AS est_price, supplier FROM requests WHERE status ="
            " 'Pending Purchaser'"
        ),
        conn,
    )
    ...
    last_po_query = (
        "SELECT pono FROM requests WHERE pono IS NOT NULL AND pono != '' ORDER"
        " BY id DESC LIMIT 1"
    )
    ...
    if pending_df.empty:
      ...
      if not pending_df.empty:
        ...
        if (
            first_row_suggestion
            and first_row_suggestion in suppliers_master
        ):
          default_vendor_index = suppliers_master.index(
              first_row_suggestion
          )
      ...
      if last_po:
        col2.caption(
            f"💡 *Last used P.O. Number was:* **{last_po}**"
        )
      else:
        col2.caption(
            "💡 *No previous P.O. numbers found in the system.*"
        )

      st.markdown("---")
      st.write("#### 📦 2. Select Items for this P.O.")
      st.info(
          "Check the **'Add to PO'** box and verify the **'Confirmed Price'**."
      )
      ...
      if st.button("✅ Submit Purchase Order", type="primary"):
        if not po_number.strip():
          ...
          if selected_items.empty:
            st.warning(
                "⚠️ You haven't selected any items to include in this P.O."
            )
          else:
            ...
            conn.commit()
            st.success(
                f"Successfully created P.O. #{po_number} with"
                f" {len(selected_items)} item(s)!"
            )
            st.rerun()
  ...
  with tab_receive:
    ...
    st.info(
        "Log items that have arrived on-site and upload attached Delivery"
        " Receipts (DR), Sales Invoices (SI), or Official Receipts (OR)."
    )
    ...
    if not pending_recv_df.empty:
      ...
      po_to_receive = recv_col1.selectbox(
          "Select PO Number", pending_recv_df["PO Number"].tolist()
      )
      ...
      if uploaded_file is not None and uploaded_file.type.startswith("image/"):
        st.image(
            uploaded_file, caption="Preview of attached document", width=250
        )

      if st.button("Confirm Receiving", type="primary"):
        if dr_number.strip():
          current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
          po_details = pending_recv_df[
              pending_recv_df["PO Number"] == po_to_receive
          ].iloc[0]
          ...
          c.execute(
              "UPDATE requests SET received_status = 'Received',"
              " received_timestamp = ? WHERE pono = ?",
              (current_time, po_to_receive),
          )

          conn.commit()
          st.session_state["receive_success_msg"] = (
              f"✅ PO #{po_to_receive} received under Doc #{dr_number}!"
      ...
      st.success(
          "🎉 No pending deliveries! All approved POs have been physically"
          " received."
      )
    ...
    if not history_df.empty:
      ...
      options = {
          f"PO #{row['PO Number']} | DR #{row['DR / SI / OR No.']} |"
          f" {row['Supplier']} ({row['Project']})": row["id"]
          for _, row in history_df.iterrows()
      }

      selected_label = st.selectbox(
          "Choose a delivery record:", list(options.keys())
      )
      selected_id = options[selected_label]

      selected_record = c.execute(
          "SELECT receipt_image, file_name FROM deliveries WHERE id = ?",
          (selected_id,),
      ).fetchone()
      ...
      if selected_record and selected_record[0]:
        ...
        if fname and fname.lower().endswith((".png", ".jpg", ".jpeg")):
          st.image(
              image_blob, caption=f"📷 Attached Receipt: {fname}", width=450
          )
          ...
          try:
            st.image(
                image_blob,
                caption=f"📷 Attached Receipt: {fname or 'Image'}",
                width=450,
            )
          except Exception:
            st.download_button(
        ...
        st.warning(
            "⚠️ No image or document file was attached for this receiving"
            " record."
        )
    else:
  ...
  if approved_pos and HAS_REPORTLAB:
    for idx, po in enumerate(approved_pos):
      ...
      col_info.write(
          f"📄 **PO Number:** {pono} | **Supplier:** {supplier} | **Project:**"
          f" {proj} | **Approved:** {app_time}"
      )

      po_items = c.execute(
          (
              "SELECT qty, unit, description, activity, price, amount FROM"
              " requests WHERE pono = ?"
          ),
          (pono,),
      ).fetchall()
      pdf_bytes = create_po_pdf(
          pono, datetime.now().strftime("%Y/%m/%d"), supplier, proj, po_items
      )
...
# --- ROLE 3: APPROVER ---
  ...
  if not pending_pos:
    ...
    for po in pending_pos:
      ...
      with st.expander(
          f"📄 PO #{pono} | Supplier: {supplier} | Project: {proj} | Submitted:"
          f" {req_time}",
          expanded=True,
      ):
        po_items_df = pd.read_sql_query(
            (
                "SELECT item_no, description, qty, unit, price, amount FROM"
                " requests WHERE pono = ? AND status = 'Pending Approval'"
            ),
            conn,
            params=(pono,),
        )
...
# --- ROLE 4: OFFICE MANAGER ---
  ...
  if not expenses_df.empty:
    pivot_exp = expenses_df.pivot_table(
        index="project_name", columns="category", values="cost", fill_value=0
    ).reset_index()
  else:
  ...
  merged_df["MATERIALS Amount"] = merged_df.get(
      "Direct Materials", 0.0
  ) + merged_df.get("Tools & Consumables", 0.0)
  merged_df["SUBCON"] = merged_df.get("Subcontract & Services", 0.0)
  merged_df["EQPT Amount"] = merged_df.get(
      "Equipment & Rental", 0.0
  ) + merged_df.get("Fuel & Lubricants", 0.0)

  merged_df["Labor Amount"] = 0.0
  merged_df["ADMIN"] = 0.0
  merged_df["REVISED Amount"] = merged_df["CONTRACT Amount"]
  merged_df["Implementing Budget Amount"] = (
      merged_df["CONTRACT Amount"] * 0.42
  )
  merged_df["Implementing Budget %"] = 0.42

  merged_df["TOTAL Amount"] = (
      merged_df["MATERIALS Amount"]
      + merged_df["Labor Amount"]
      + merged_df["ADMIN"]
      + merged_df["SUBCON"]
      + merged_df["EQPT Amount"]
  )

  for col, target in [
      ("MATERIALS Amount", "Mat %"),
      ("Labor Amount", "Lab %"),
      ("EQPT Amount", "Eqpt %"),
      ("TOTAL Amount", "Total %"),
  ]:
    merged_df[target] = (
        (merged_df[col] / merged_df["Implementing Budget Amount"])
...
# --- ROLE 5: ACCOUNTING ---
        ...
        if submitted_coa:
          if new_acc_code.strip() and new_acc_name.strip():
            try:
              ...
              st.error(
                  f"⚠️ Account Code '{new_acc_code.strip()}' already exists."
              )
          else:
        ...
        if apv_input.strip():
          ...
          c.execute(
              ...
              (
                  ...
                  dr_to_apv,
                  (
                      f"APV setup for DR #{dr_to_apv}"
                      f" ({selected_del['Supplier']})"
                  ),
              ),
          )
          ...
          c.execute(
              ...
              (
                  ...
                  total_amt,
                  dr_to_apv,
                  f"APV liability accrued for DR #{dr_to_apv}",
              ),
          )

          conn.commit()
          st.success(
              f"🎉 Voucher {apv_input.strip()} recorded successfully for DR"
              f" #{dr_to_apv}!"
          )
          st.rerun()
        else:
  ...
  with tab_payment:
    ...
    if not pay_df.empty:
      st.dataframe(
          pay_df.style.format({"Total Amount": "₱{:,.2f}"}),
          use_container_width=True,
          hide_index=True,
      )
      ...
      apv_to_pay = col1.selectbox(
          "Select APV Number to Pay", pay_df["APV Number"].tolist()
      )
      ...
      bank_choice = st.selectbox(
          "Select Funding Cash/Bank Account",
      ...
      if st.button("✅ Process Payment & Issue Voucher", type="primary"):
        if cv_input.strip():
          ...
          c.execute(
              "UPDATE requests SET payment_status = 'Paid' WHERE pono = ?",
              (selected_pay["PO Number"],),
          )
          ...
          c.execute(
              ...
              (
                  ...
                  apv_to_pay,
                  (
                      f"Settlement of APV #{apv_to_pay}"
                      f" ({selected_pay['Supplier']})"
                  ),
              ),
          )
          ...
          c.execute(
              ...
              (
                  ...
                  pay_amt,
                  apv_to_pay,
                  f"Payment release via {pay_method}",
              ),
          )

          conn.commit()
          st.success(
              f"🎉 Voucher {cv_input.strip()} completed! Payment cleared for APV"
              f" #{apv_to_pay}."
          )
          st.rerun()
        else:
  ...
  with tab_gl:
    ...
    if not gl_df.empty:
      ...
      st.write(
          f"**Total Debits:** ₱{d_sum:,.2f} | **Total Credits:** ₱{c_sum:,.2f}"
      )
    else:
      st.info(
          "No journal entries posted yet. Generate an APV or CV to trigger"
          " automated entries."
      )
...
# --- ROLE 6: ADMIN VIEW ALL ---
  ...
  tab_settings, tab_reports, tab_payables = st.tabs([
      "⚙️ Master Database & Settings",
      "📊 Project Approved Reports",
      "💸 Accounts Payable",
  ])
  ...
  with tab_settings:
    ...
    with st.expander("👥 Manage System Users"):
      ...
      users_df = pd.read_sql_query(
          (
              "SELECT id, username, password, role1, role2, role3, role4, role5,"
              " role6, status, can_add_act, can_add_item FROM users"
          ),
          conn,
      )
      ...
      edited_users = st.data_editor(
          ...
          column_config={
              ...
              "role1": st.column_config.SelectboxColumn(
                  "Role 1", options=role_options
              ),
              "role2": st.column_config.SelectboxColumn(
                  "Role 2", options=role_options
              ),
              "role3": st.column_config.SelectboxColumn(
                  "Role 3", options=role_options
              ),
              "role4": st.column_config.SelectboxColumn(
                  "Role 4", options=role_options
              ),
              "role5": st.column_config.SelectboxColumn(
                  "Role 5", options=role_options
              ),
              "role6": st.column_config.SelectboxColumn(
                  "Role 6", options=role_options
              ),
              "status": st.column_config.SelectboxColumn(
                  "Status", options=["Active", "Inactive"], default="Active"
              ),
              "can_add_act": st.column_config.SelectboxColumn(
                  "Can Add Act?", options=yes_no_options, default="No"
              ),
              "can_add_item": st.column_config.SelectboxColumn(
                  "Can Add Item?", options=yes_no_options, default="No"
              ),
          },
      )
      ...
      if st.button("💾 Save User Changes", type="primary"):
        ...
        for _, row in edited_users.iterrows():
          if pd.notnull(row["username"]) and str(row["username"]).strip() != "":
            r1 = (
                str(row.get("role1", ""))
                if pd.notnull(row.get("role1"))
                else ""
            )
            r2 = (
                str(row.get("role2", ""))
                if pd.notnull(row.get("role2"))
                else ""
            )
            r3 = (
                str(row.get("role3", ""))
                if pd.notnull(row.get("role3"))
                else ""
            )
            r4 = (
                str(row.get("role4", ""))
                if pd.notnull(row.get("role4"))
                else ""
            )
            r5 = (
                str(row.get("role5", ""))
                if pd.notnull(row.get("role5"))
                else ""
            )
            r6 = (
                str(row.get("role6", ""))
                if pd.notnull(row.get("role6"))
                else ""
            )
            status = (
                str(row.get("status", "Active"))
                if pd.notnull(row.get("status"))
                else "Active"
            )
            can_add_act = (
                str(row.get("can_add_act", "No"))
                if pd.notnull(row.get("can_add_act"))
                else "No"
            )
            can_add_item = (
                str(row.get("can_add_item", "No"))
                if pd.notnull(row.get("can_add_item"))
                else "No"
            )
    ...
    with st.expander("🏗️ Manage Project Activities"):
      ...
      projects_list = [
          p[0] for p in c.execute("SELECT project_name FROM projects").fetchall()
      ]
      sel_proj_act = st.selectbox(
          "Select Project for Activity", projects_list, key="sel_proj_editable"
      )

      if sel_proj_act:
        proj_row = c.execute(
            "SELECT id FROM projects WHERE project_name = ?", (sel_proj_act,)
        ).fetchone()
        if proj_row:
          ...
          if not acts_df.empty:
            ...
            col_metric.markdown(
                f"**Total Contract:** ₱{total_contract:,.2f} | **Total"
                f" Implementing (42%):** ₱{total_implementing:,.2f}"
            )
          else:
          ...
          new_qty = ac2.number_input(
              "Qty", min_value=0.0, value=1.0, step=1.0, key="new_act_qty"
          )
          new_unit = ac3.text_input("Unit", value="lot", key="new_act_unit")
          new_amount = ac4.number_input(
              "Contract Amount (₱)",
              min_value=0.0,
              step=1000.0,
              format="%.2f",
    ...
    with st.expander("✍️ Manage Signatories & Signatures"):
      ...
      signatories_df = pd.read_sql_query(
          "SELECT id, name, role, signature_path FROM signatories", conn
      )
      ...
      edited_sigs_df = st.data_editor(
          ...
          column_config={
              ...
              "role": st.column_config.SelectboxColumn(
                  "Role", options=["Preparer", "Approver"], required=True
              ),
      ...
      if st.button(
          "💾 Save Signatories Changes", type="primary", key="save_sigs_btn"
      ):
    ...
    with st.expander("📦 Add New Material Item"):
      all_items = [
          r[0]
          for r in c.execute("SELECT item_no FROM materials").fetchall()
          if r[0] and r[0].isdigit()
      ]
      suggested_item_no = (
          str(max([int(x) for x in all_items]) + 1).zfill(5)
          if all_items
          else "00001"
      )
    ...
    with st.expander("🚚 Add & Manage Suppliers"):
      ...
      selected_supplier_to_edit = st.selectbox(
          "Select Supplier to Edit", supplier_edit_options
      )

      edit_name, edit_loc, edit_contact_person, edit_contact_no, edit_tin, edit_vat, edit_terms = (
          "",
          "",
          "",
          "",
      ...
      if selected_supplier_to_edit != "-- Register New Supplier --":
        sup_data = c.execute(
            (
                "SELECT supplier_name, location, contact_person, contact_number,"
                " tin_number, vat_type, terms_days FROM suppliers WHERE"
                " supplier_name = ?"
            ),
      ...
      sup_contact_person = sc3.text_input(
          "Contact Person", value=edit_contact_person
      )
      sup_contact_number = sc4.text_input(
          "Contact Number", value=edit_contact_no
      )

      sc5, sc6 = st.columns(2)
      sup_tin = sc5.text_input("TIN Number", value=edit_tin)
      sup_vat = sc6.selectbox(
          "VAT Status",
          ["VAT Registered", "Non-VAT"],
          index=0 if edit_vat == "VAT Registered" else 1,
      )
      sup_terms = st.number_input(
          "Payment Terms (Days)", min_value=0, value=edit_terms, step=1
      )

      btn_label = (
          "Update Supplier Details"
          if selected_supplier_to_edit != "-- Register New Supplier --"
          else "Save New Supplier"
      )
        ...
        if sup_name.strip():
          c.execute(
              ...
              (
                  sup_name.strip(),
                  sup_location.strip(),
                  sup_contact_person.strip(),
                  sup_contact_number.strip(),
                  sup_tin.strip(),
                  sup_vat,
                  sup_terms,
              ),
    ...
    st.dataframe(
        pd.read_sql_query("SELECT * FROM requests ORDER BY id DESC", conn),
        use_container_width=True,
        hide_index=True,
    )
  ...
  with tab_reports:
    ...
    projects_query = (
        "SELECT DISTINCT project_name FROM requests WHERE status = 'Approved /"
        " Ongoing' AND project_name IS NOT NULL AND project_name != ''"
    )
    suppliers_query = (
        "SELECT DISTINCT supplier FROM requests WHERE status = 'Approved /"
        " Ongoing' AND supplier IS NOT NULL AND supplier != ''"
    )

    available_projects = ["All Projects"] + [
        p[0] for p in c.execute(projects_query).fetchall()
    ]
    available_suppliers = ["All Suppliers"] + [
        s[0] for s in c.execute(suppliers_query).fetchall()
    ]

    col_f1, col_f2 = st.columns(2)
    selected_project = col_f1.selectbox(
        "📌 Filter by Project Name:", available_projects
    )
    selected_supplier = col_f2.selectbox(
        "🚚 Filter by Supplier:", available_suppliers
    )
    ...
    if approved_df.empty:
      ...
      m1.metric(
          "💰 Total Approved Cost",
          f"₱{approved_df['Total Amount'].sum():,.2f}",
      )
      m2.metric(
          "📦 Total Approved Quantity",
          f"{approved_df['Qty'].sum():,.0f} units",
      )
      m3.metric(
          "📄 Total Approved P.O.s",
          f"{approved_df['P.O. Number'].nunique()} Orders",
      )

      st.markdown("---")
      st.dataframe(
          approved_df.style.format(
              {"Unit Price": "₱{:,.2f}", "Total Amount": "₱{:,.2f}"}
          ),
          use_container_width=True,
          hide_index=True,
      )
  ...
  with tab_payables:
    ...
    if not ap_df.empty:
      ...
      ap_df["Due Date"] = (
          ap_df["Date Received"]
          + pd.to_timedelta(ap_df["Terms (Days)"], unit="D")
      ).dt.strftime("%Y-%m-%d")
      ...
      st.error(
          f"**Total Outstanding Payables:** ₱ {ap_df['Total Amount'].sum():,.2f}"
      )

      st.markdown("---")
      col_pay1, col_pay2 = st.columns([2, 1])
      dr_to_pay = col_pay1.selectbox(
          "Select DR Number to mark as Paid", ap_df["DR Number"].tolist()
      )

      if col_pay2.button("Confirm Direct Settlement", type="primary"):
        c.execute(
            "UPDATE deliveries SET payment_status = 'Paid' WHERE dr_number = ?",
            (dr_to_pay,),
        )
        conn.commit()
